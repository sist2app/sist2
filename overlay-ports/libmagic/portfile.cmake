...
if(VCPKG_CROSSCOMPILING AND NOT VCPKG_HOST_IS_WINDOWS)
    # magic.mgc is compiled by running `file`, so a cross build reaches for the host one on PATH.
    # It looks it up under the target's EXEEXT, which is .exe for a Windows target.
    if(VCPKG_TARGET_IS_WINDOWS)
        set(target_exeext ".exe")
    else()
        set(target_exeext "")
    endif()
    set(host_file_bin "${CURRENT_BUILDTREES_DIR}/host-tool")
    file(MAKE_DIRECTORY "${host_file_bin}")
    file(COPY_FILE "${CURRENT_HOST_INSTALLED_DIR}/tools/libmagic/bin/file" "${host_file_bin}/file${target_exeext}")
    vcpkg_add_to_path(PREPEND "${host_file_bin}")
